  ...
  async store(input: StorePackageInput): Promise<StoredPackage> {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      try {
        ...
        // its hash is kept, so a customer who never receives it has to ask for
        // a new one (`ReissuePickupCodeService`) rather than be told this one.
        // Sending must not fail the drop — the parcel is already in the locker
        // and the agent has walked away — so a real adapter hands off to a
        // queue and retries there rather than throwing into this path.
        return {
          packageId: pkg.id,
